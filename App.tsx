...
const App: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-[#f4f7fa] overflow-hidden font-sans" dir="rtl">
        ...
        <header className="h-24 bg-white border-b flex items-center justify-between px-10 z-50 sticky top-0 shadow-sm">
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
               <h1 className="font-black text-[#002d72] text-2xl tracking-tight">דו"ח ניהולי</h1>
               <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Bank Pro Analytics</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 bg-slate-100 p-1.5 rounded-[1.5rem] shadow-inner">
             <button 
                ...
                className={`px-6 py-2 rounded-[1.2rem] text-[11px] font-black transition-all ${!editMode ? 'bg-white text-[#002d72] shadow-md' : 'text-slate-500 hover:text-slate-800'}`}
             >
                צפייה (View)
             </button>
             <button 
          ...
          <div className="flex gap-2">
            {editMode && (
              <>
                <button className="px-5 py-3 bg-white border border-slate-200 text-slate-700 rounded-2xl text-[11px] font-black hover:bg-slate-50 transition-all shadow-sm" onClick={() => setShowJsonModal(true)}>JSON</button>
                <button className="px-5 py-3 bg-white border border-slate-200 text-slate-700 rounded-2xl text-[11px] font-black hover:bg-slate-50 transition-all shadow-sm" onClick={() => fileInputRef.current?.click()}>ייבוא</button>
                <button className="px-5 py-3 bg-rose-50 text-rose-600 rounded-2xl text-[11px] font-black hover:bg-rose-600 hover:text-white transition-all" onClick={handleResetReport}>נקה</button>
              </>
            )}
            <button className="px-8 py-3 bg-[#002d72] text-white rounded-2xl text-[11px] font-black shadow-xl hover:bg-blue-600 transition-all" onClick={handleExport}>ייצוא</button>
          </div>
        </header>
          ...
          <div className={`max-w-[1400px] mx-auto grid grid-cols-12 gap-10 pb-32`}>
            {report.sections.length === 0 ? (
              <div className="col-span-12 h-80 flex flex-col items-center justify-center border-4 border-dashed rounded-[4rem] text-slate-300 bg-white/50 border-slate-200">
                ...
                <p className="text-xl font-black">הדו"ח ריק</p>
