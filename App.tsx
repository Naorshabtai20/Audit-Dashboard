...
const App: React.FC = () =>
{
  ...
  // refs for each rendered section element so we can scroll them into view when selected
  ...
  // When the selectedId changes, attempt to scroll the corresponding section into view
  useEffect(() =>
  {
    if (!selectedId) return;
    const el = sectionElementRefs.current[selectedId];
    if (el && typeof el.scrollIntoView === 'function')
    {
      // scroll smoothly and center the element in the viewport
      el.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' });
    }
  }, [selectedId, activeTabIndex]);

  // Clear stored refs when changing tabs to avoid stale references
  useEffect(() =>
  {
    sectionElementRefs.current = {};
  }, [activeTabIndex]);
          ...
          {report.tabs.map((tab, idx) => (
            <div key={idx} className="relative group w-full flex justify-center">
              <button
                ...
                className={`w-16 h-16 rounded-3xl flex flex-col items-center justify-center transition-all duration-300 ${activeTabIndex === idx
            ...
            {activeTab.sections.length === 0 ? (
              ...
              activeTab.sections.map((section) => (
                <div
                  ...
                  ref={el => { sectionElementRefs.current[section.id] = el; }}
