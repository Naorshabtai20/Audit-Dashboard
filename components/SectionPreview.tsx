...
import { Section, TextSection, ChartSection, TableSection, KPISection, GraphicSection as GraphicSection, SummaryEvaluationSection, DateSection, AnomalySection } from '../types';
import
{
...
const Card: React.FC<{
  ...
  {
    const isFixedHeight = type === 'chart' || type === 'graphic';
    ...
    return (
      <div
        ...
        className={`group transition-all duration-500 relative w-full flex flex-col overflow-visible ${pill ? 'rounded-[1.2rem]' : 'rounded-[1.2rem]'
          } ${isSelected
...
export const SectionPreview: React.FC<{
...
}> = ({ section, isSelected, onDelete, onSelect }) =>
  {
    ...
    const renderSummaryEvaluation = (sec: SummaryEvaluationSection) => (
      <div className="flex flex-col w-full text-right gap-6 p-6" style={{ overflowWrap: 'anywhere', wordBreak: 'break-word' }}>
        <div className="flex flex-wrap lg:flex-nowrap gap-y-6 gap-x-6 shrink-0">
          <div className="flex-grow basis-[300px] bg-white rounded-[1.2rem] shadow-sm border border-slate-100 flex flex-col justify-center p-8">
            <p className="text-[#002d72] font-black italic leading-tight text-right" style={{ fontSize: `${1.1 * dataFontScale}rem` }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="min-w-[280px] bg-[#f8fbff] rounded-[1.2rem] shadow-inner border border-emerald-50 p-8">
            <h3 className="font-black text-emerald-700" style={{ fontSize: `${1.1 * fontScale}rem`, marginBottom: '0.8rem' }}>המלצות הביקורת</h3>
            <ul className="flex flex-col gap-2 items-end pr-2 pb-2" dir="rtl">
              {(sec.recommendations || []).map((item, idx) => (
                <li key={idx} className="flex items-start gap-2 flex-row-reverse justify-end w-full">
                  <p className="text-slate-700 font-bold leading-snug text-right w-full" style={{ fontSize: `${0.95 * dataFontScale}rem` }}>{item}</p>
                  <span className="text-emerald-500 font-black" style={{ fontSize: `${1 * dataFontScale}rem` }}>✓</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="min-w-[280px] bg-[#fff8f8] rounded-[1.2rem] shadow-inner border border-rose-50 p-8">
            ...
            <ul className="flex flex-col gap-2 items-end pr-2 pb-2" dir="rtl">
              {(sec.deficiencies || []).map((item, idx) => (
                <li key={idx} className="flex items-center gap-2 flex-row-reverse justify-end w-full">
                  <p className="text-slate-700 font-black leading-snug text-right w-full" style={{ fontSize: `${0.95 * dataFontScale}rem` }}>{item}</p>
                  <span className="font-black italic text-slate-200 leading-none whitespace-nowrap flex-shrink-0" style={{ fontSize: `${1.5 * dataFontScale}rem` }}>{(idx + 1).toString()}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          return (
            <div key={item.id} className="flex flex-col gap-6">
              ...
              <div
                ...
                className={`transition-all duration-300 relative rounded-[1.2rem] border-2 p-6 flex items-center justify-between cursor-pointer ${isExpanded ? 'bg-blue-50/30 border-blue-400 shadow-xl' : 'bg-white border-blue-100 hover:border-blue-200 shadow-sm'
                  }`}
              >
                {/* spacer (left) - bars moved next to status */}
                <div className="w-2" />

                {/* Center: Title and info */}
                <div className="flex-1 flex flex-col min-w-0">
                  <h3 className="font-black text-[#002d72] truncate" style={{ fontSize: `${1.1 * fontScale}rem` }}>{item.title}</h3>
                  <div className="flex items-end mt-1 gap-6">
                    <div className="flex items-end gap-2 h-6">
                      <div className="flex gap-1 items-end" style={{ height: '18px' }}>
                        {[1, 2, 3, 4, 5].map(dot => (
                          <div key={dot} className={`rounded-sm ${dot <= item.riskLevel ? 'bg-rose-500' : 'bg-slate-200'}`} style={{ width: '0.45rem', height: dot <= item.riskLevel ? '12px' : '7px' }}></div>
                        ))}
                      </div>
                      <span className="text-[10px] font-bold text-slate-400 whitespace-nowrap self-end">ציון סיכון</span>
                    </div>
                    <div className="flex items-end gap-1 h-6">
                      <span className="text-[10px] font-bold text-slate-400 whitespace-nowrap self-end">סטטוס:</span>
                      <span className="text-[10px] font-black text-blue-600 whitespace-nowrap self-end">{item.status}</span>
                    </div>
                    <div className="flex items-end gap-1 px-3 py-0 bg-slate-100 rounded-lg h-6">
                      <span className="text-[10px] font-bold text-slate-400 whitespace-nowrap self-end">גורם מבוקר:</span>
                      <span className="text-[10px] font-black text-slate-600 whitespace-nowrap self-end">{item.department}</span>
                    </div>
                  </div>
                </div>
              ...
              {/* Expanded details */}
              {isExpanded && (
                <div className="bg-slate-50/50 rounded-[1.2rem] p-10 animate-in slide-in-from-top-4 duration-500 flex flex-col gap-10">
                  {/* Main content row */}
                  <div className="grid grid-cols-12 gap-8">
                    {/* Left side card: Risk factor analysis */}
                    <div className="col-span-12 lg:col-span-5 bg-white p-8 rounded-[1.2rem] shadow-sm border border-slate-100 flex flex-col gap-4 text-right">
                      <span className="font-black text-slate-400 uppercase tracking-widest block text-[11px] border-b pb-2">ניתוח גורמי סיכון</span>
                    ...
                    <div className="col-span-12 lg:col-span-7 bg-white p-8 rounded-[1.2rem] shadow-sm border border-slate-100 flex flex-col gap-4 text-right">
                      <span className="font-black text-blue-600 uppercase tracking-widest block text-[11px] border-b pb-2">דוח חריגה מפורט</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="bg-white p-6 rounded-[1.2rem] shadow-sm border border-slate-100 flex flex-col gap-2 items-center text-center">
                      <span className="font-black text-slate-400 uppercase tracking-widest text-[10px]">מזהה פנימי</span>
                      <span className="font-black text-[#002d72] text-xl tracking-tight">{item.internalRef || 'N/A'}</span>
                    </div>
                    <div className="bg-blue-50/50 p-6 rounded-[1.2rem] shadow-sm border border-blue-100 flex flex-col gap-2 items-center text-center">
                      <span className="font-black text-blue-500 uppercase tracking-widest text-[10px]">סטטוס פרוטוקול</span>
                      <span className="font-black text-blue-900 text-xl">{item.protocolStatus || 'בטיפול'}</span>
                    </div>
                    <div className="bg-white p-6 rounded-[1.2rem] shadow-sm border border-slate-100 flex flex-col gap-2 items-center text-center col-span-1 lg:col-span-2 overflow-hidden">
                      <span className="font-black text-indigo-500 uppercase tracking-widest text-[10px]">קישור / משאבים</span>
                      {item.link ? (
    ...
    const renderChart = (sec: ChartSection) =>
    {
    ...
    return (
      <Card
        ...
        noPadding={['summary_evaluation', 'kpi', 'graphic'].includes(section.type)}
        pill={section.type === 'date'}
      ...
      >
        <div className="flex flex-col h-full w-full" style={{ overflowWrap: 'anywhere', wordBreak: 'break-word' }}>
          {section.title && !['date', 'anomaly', 'graphic'].includes(section.type) && (
            <h2
              ...
              style={{
                ...
                marginBottom: '1.2rem',
                ...(section.type === 'kpi' || section.type === 'summary_evaluation' ? { marginRight: '1.5rem', marginTop: '0.5rem' } : {})
              }}
            >
              {section.title}
            </h2>
          ...
          {section.type === 'chart' && renderChart(section as ChartSection)}
          ...
          {section.type === 'date' && (
            <div className="flex items-center gap-4">
              <div className="bg-blue-50 rounded-[1.2rem] flex items-center justify-center shrink-0 border border-blue-100 w-14 h-14 shadow-sm">
                <span className="text-3xl">📅</span>
              </div>
              <div className="flex flex-col text-right">
                <span className="font-black text-slate-400 uppercase tracking-widest" style={{ fontSize: `${11 * labelFontScale}px` }}>{(section as DateSection).label}</span>
                <span className="font-black text-[#002d72] tracking-tighter" style={{ fontSize: `${2 * dataFontScale}rem` }}>{(section as DateSection).date.split('-').reverse().join('/')}</span>
              </div>
            </div>
          )}

          {section.type === 'graphic' && (section as GraphicSection).src && (
            <div className="w-full relative flex-1 min-h-0 bg-slate-50 rounded-[1.2rem] overflow-hidden">
              <img src={(section as GraphicSection).src} className="w-full h-full object-contain" alt="Graphic" />
              {section.title && <div className="absolute bottom-4 right-4 bg-white/90 backdrop-blur-md px-4 py-2 rounded-full font-bold shadow-lg border border-white/50" style={{ fontSize: `${12 * labelFontScale}px` }}>{section.title}</div>}
            </div>
          )}
        </div>
      </Card>
