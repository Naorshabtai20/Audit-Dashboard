...
export const SectionPreview: React.FC<{ 
  ...
  const renderSummaryEvaluation = (sec: SummaryEvaluationSection) => (
    <div className="grid grid-cols-12 gap-6 w-full h-full p-8 bg-[#f8fbff] overflow-y-auto custom-scrollbar">
      ...
      <div className="col-span-12 lg:col-span-6 bg-[#fff8f8] rounded-[2.5rem] p-10 border border-rose-100 shadow-sm flex flex-col min-h-[300px]">
        ...
        <div className="space-y-6 flex-1 overflow-y-auto custom-scrollbar pr-2">
           {sec.deficiencies.length > 0 ? sec.deficiencies.map((d, i) => (
             <div key={i} className="flex items-start gap-5 text-right">
                <span className="text-rose-200 font-black italic text-2xl leading-none">0{i+1}</span>
                <p className="font-bold text-[#440000] text-sm leading-relaxed">{d}</p>
             </div>
           )) : <p className="text-slate-300 italic text-sm">טרם הוזנו ליקויים</p>}
        </div>
      </div>

      <div className="col-span-12 lg:col-span-6 bg-[#f8fffa] rounded-[2.5rem] p-10 border border-emerald-100 shadow-sm flex flex-col min-h-[300px]">
        <div className="flex items-center gap-3 mb-8">
        ...
        <div className="space-y-6 flex-1 overflow-y-auto custom-scrollbar pr-2">
           {sec.recommendations.length > 0 ? sec.recommendations.map((r, i) => (
             <div key={i} className="flex items-start gap-4 text-right">
                ...
                <p className="font-bold text-[#003322] text-sm leading-relaxed">{r}</p>
             </div>
  ...
  const renderChart = (sec: DataChartSection) => {
    // We filter the keys to ONLY include what's actually in the data points
    // This prevents showing old/irrelevant series from previous pastes
