...
export const EditorPanel: React.FC<EditorPanelProps> = ({ report, onUpdate, selectedId, onSelect, onMove, onCloseSidebar }) => {
  ...
  useEffect(() => {
    ...
    } else if (selectedSection.type === 'data_chart') {
  ...
  const supportsManualHeight = selectedSection?.type === 'data_chart' || selectedSection?.type === 'pasted_graphic';
          ...
          {SECTION_OPTIONS.map(opt => (
            <button key={opt.type} onClick={() => {
                ...
                const newSec = { 
                  ...
                  styles: { colSpan: 12, height: (opt.type === 'data_chart' || opt.type === 'pasted_graphic') ? 400 : undefined, fontScale: 1, dataFontScale: 1, labelFontScale: 1, alignment: 'right' },
                ...
                <>
                    ...
                    {selectedSection.type === 'date_picker' && (
                        <div className="space-y-6 text-right">
                            ...
                            <input type="text" value={(selectedSection as DatePickerSection).label} onChange={e => updateSection(selectedId!, { label: e.target.value })} className="w-full p-4 border rounded-2xl font-bold mb-4" />
                            <input type="date" value={(selectedSection as DatePickerSection).date} onChange={e => updateSection(selectedId!, { date: e.target.value })} className="w-full p-6 border-4 border-indigo-100 rounded-[2.5rem] font-black text-2xl text-center text-indigo-600" />
                        </div>
                    )}
